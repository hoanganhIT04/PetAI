    ...
    <div class="pt-32 pb-20 px-4 min-h-screen bg-slate-50">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="text-center mb-10">
                <h1 class="text-3xl md:text-4xl font-black text-slate-900 mb-4 tracking-tight">Thư viện <span
                        class="text-teal-600">Thú Cưng</span></h1>
                <p class="text-base md:text-lg text-slate-500 font-medium max-w-2xl mx-auto">Khám phá thông tin chi tiết
                    về hàng trăm giống chó mèo phổ biến nhất hiện nay.</p>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <aside
                    class="w-full lg:w-72 bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm shrink-0 sticky top-24 hidden lg:block">
                    ...
                    <div v-if="false">
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            <div class="bg-slate-100 px-4 py-2 rounded-xl font-bold text-slate-700 w-full text-center">
                                {{ priceRange[0] }}M</div>
                            <span class="font-bold text-slate-300">-</span>
                            <div class="bg-slate-100 px-4 py-2 rounded-xl font-bold text-slate-700 w-full text-center">
                                {{ priceRange[1] }}M</div>
                        </div>
                        <input type="range" v-model.number="priceRange[0]" min="0" max="50" step="1"
                            class="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-teal-600 mb-2">
                        <input type="range" v-model.number="priceRange[1]" min="0" max="100" step="1"
                ...
                <main class="flex-1 w-full">
                    ...
                    <div class="lg:hidden mb-6 flex gap-4">
                        ...
                        <button @click="showMobileFilters = true"
                            ...
                            <span v-if="activeFiltersCount > 0"
                                class="absolute -top-2 -right-2 bg-orange-500 text-white w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center border-2 border-white">
                                {{ activeFiltersCount }}
                            </span>
                        </button>
                    </div>
                    ...
                    <div class="hidden lg:block relative w-full mb-8">
                        <Search
                            class="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 w-6 h-6 group-focus-within:text-teal-600 transition" />
                        <input v-model="searchQuery" type="text" placeholder="Tìm kiếm tên giống thú cưng..."
                    ...
                    <div v-if="filteredPets.length > 0"
                        class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-12">
                    ...
                    <div v-else
                        ...
                        <button @click="clearFilters"
                            class="px-8 py-3 bg-teal-100 text-teal-700 font-bold rounded-xl hover:bg-teal-200 transition">
                            Xóa bộ lọc
                        </button>
                    </div>
        ...
        <div v-if="showMobileFilters" class="fixed inset-0 z-50 lg:hidden">
            ...
            <div class="absolute right-0 top-0 bottom-0 w-80 bg-white shadow-2xl p-6 overflow-y-auto">
                ...
                <button @click="clearFilters(); showMobileFilters = false"
                    class="w-full py-4 text-orange-500 font-bold border-2 border-orange-100 rounded-xl">
                    Xóa bộ lọc
                </button>
            </div>
        </div>
