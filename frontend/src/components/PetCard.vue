...
<template>
  <div
    ...
    <div class="h-48 overflow-hidden relative">
      <img :src="pet.image_path" class="w-full h-full object-cover group-hover:scale-110 transition duration-500"
        alt="Pet Image" @error="$event.target.src = '/assets/placeholder_pet.jpg'">
      <div
        class="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-[10px] font-black uppercase text-teal-600 shadow-sm">
        {{
          pet.size === 'small' ? 'Nhỏ' :
    ...
    <div class="p-5">
      ...
      <div class="flex items-center gap-3 text-slate-500 text-xs mb-3">
        <span class="flex items-center gap-1">
          <Clock class="w-3 h-3" /> {{ pet.lifespan }}
        </span>
      </div>
