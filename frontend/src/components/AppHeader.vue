...
import { Dog, Cat, ScanLine, Heart, BookOpen, CheckCircle } from 'lucide-vue-next'

const route = useRoute()
</script>

<template>
  <header class="fixed w-full top-0 left-0 z-50 transition-all duration-300 glass border-b border-slate-200"
    id="main-header">
    <div class="max-w-7xl mx-auto px-4 h-16 grid grid-cols-3 items-center">
      ...
      <RouterLink to="/" class="flex items-center gap-3 group justify-self-start">
        <div
          class="relative w-8 h-8 bg-teal-600 rounded-lg flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform">
        ...
        <span class="text-2xl font-extrabold text-slate-800 tracking-tight">Pet<span
            class="text-teal-600">AI</span></span>
      </RouterLink>

      <!-- Desktop Nav (Center) -->
      <nav
        class="hidden md:flex items-center gap-8 bg-white/50 px-6 py-2 rounded-full border border-slate-100 shadow-sm backdrop-blur-md justify-self-center">
        <RouterLink to="/"
